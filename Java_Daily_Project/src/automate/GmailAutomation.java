...
public class GmailAutomation extends JFrame {
    ...
    private void initUI() {
        ...
        sendButton.addActionListener(e -> {
            ...
            new Thread(this::sendEmails).start();
        });
        statusPanel.add(sendButton, BorderLayout.SOUTH);
    ...
    private JPanel createBasicPanel() {
        ...
        JPanel chromeSettingsPanel = createStyledPanel("Chrome Settings");
        chromeSettingsPanel.add(new JLabel("Chrome Path:"), createGbc(0, 0));
        chromePathField = new JTextField(30);
        chromePathField.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        chromePathField.setText("C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe");
        JPanel chromePathPanel = new JPanel(new BorderLayout());
        chromePathPanel.add(chromePathField, BorderLayout.CENTER);
        JButton browseChromeButton = new JButton("Browse");
        browseChromeButton.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        browseChromeButton.addActionListener(e -> selectChromePath());
        chromePathPanel.add(browseChromeButton, BorderLayout.EAST);
        chromeSettingsPanel.add(chromePathPanel, createGbc(1, 0));
        panel.add(chromeSettingsPanel, gbc);
        
        // Recipients Panel
        JPanel recipientsPanel = createStyledPanel("Recipients");
        recipientsPanel.add(new JLabel("Recipients File:"), createGbc(0, 0));
        recipientsFileField = new JTextField(30);
        recipientsFileField.setEditable(false);
        recipientsFileField.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        JPanel recipientsFilePanel = new JPanel(new BorderLayout());
        recipientsFileLabel = new JLabel("No file selected");
        recipientsFileLabel.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        JButton browseRecipientsButton = new JButton("Browse");
        browseRecipientsButton.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        browseRecipientsButton.addActionListener(e -> selectRecipientsFile());
        recipientsFilePanel.add(recipientsFileLabel, BorderLayout.CENTER);
        recipientsFilePanel.add(browseRecipientsButton, BorderLayout.EAST);
        recipientsPanel.add(recipientsFilePanel, createGbc(1, 0));
        recipientsPanel.add(new JLabel("Or enter recipients (comma separated):"), createGbc(0, 1));
        recipientsField = new JTextField(30);
        recipientsField.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        recipientsPanel.add(recipientsField, createGbc(1, 1));
        gbc.gridy = 1;
        panel.add(recipientsPanel, gbc);
        
        // Email Content Panel
        JPanel emailContentPanel = createStyledPanel("Email Content");
        emailContentPanel.add(new JLabel("Subject:"), createGbc(0, 0));
        subjectField = new JTextField(30);
        subjectField.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        emailContentPanel.add(subjectField, createGbc(1, 0));
        emailContentPanel.add(new JLabel("Body:"), createGbc(0, 1));
        bodyArea = new JTextArea(10, 30);
        bodyArea.setLineWrap(true);
        bodyArea.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        JScrollPane scrollPane = new JScrollPane(bodyArea);
        GridBagConstraints bodyGbc = createGbc(1, 1);
        bodyGbc.fill = GridBagConstraints.BOTH;
        bodyGbc.weighty = 1.0;
        emailContentPanel.add(scrollPane, bodyGbc);
        emailContentPanel.add(new JLabel("Attachment:"), createGbc(0, 2));
        JPanel attachmentPanel = new JPanel(new BorderLayout());
        attachmentLabel = new JLabel("No file selected");
        attachmentLabel.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        attachmentButton = new JButton("Browse");
        attachmentButton.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        attachmentButton.addActionListener(e -> selectAttachment());
        attachmentPanel.add(attachmentLabel, BorderLayout.CENTER);
        attachmentPanel.add(attachmentButton, BorderLayout.EAST);
        emailContentPanel.add(attachmentPanel, createGbc(1, 2));
        gbc.gridy = 2;
        gbc.weighty = 1.0;
        panel.add(emailContentPanel, gbc);
    ...
    private void sendEmails() {
        ...
        if (chromePath.isEmpty()) {
            showErrorDialog("Please specify Chrome executable path.");
        ...
        if (recipients.isEmpty()) {
            showErrorDialog("Please select a recipients file or enter at least one recipient email.");
            sendButton.setEnabled(true);
            return;
        }
    ...
    private List<String> readRecipientsFromFile() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(selectedRecipientsFile))) {
            ...
            showErrorDialog("Error reading recipients file: " + e.getMessage());
        }
        return recipients;
    }
